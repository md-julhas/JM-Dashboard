...
const Starred = () => {
  ...
  return (
    <div className="overflow-y-auto h-full w-full bg-lightPrimary dark:bg-darkThird rounded-md shadow-sm">
      {/* If starred emails exist then render this */}
      {emailList.length > 0 && (
        <EmailControlBar
          emailList={emailList}
      ...
      {/* Email lists */}
      {emailList.length === 0 ? (
        // No starred emails message
        <div className="flex flex-col items-center justify-center h-full space-y-4 px-5">
          <h2 className="text-3xl font-semibold">No Starred Emails</h2>
          <p className="text-lg text-center">
            You have no starred emails at the moment. Enjoy your day!
          </p>
          <div className="flex items-center justify-center w-16 h-16 rounded-full bg-gray-200 dark:bg-gray-700">
            <FaRegStar className="text-4xl text-gray-400 dark:text-darkText" />
          </div>
